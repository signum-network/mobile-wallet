...
export function formatAttachmentData(jsonString: string): string {
  try {
    ...
    if (json['version.Message']) {
      try {
        return json.messageIsText
          ? json.message
          : convertHexStringToString(json.message);
      } catch (e) {
        return json.message;
      }
    }
    ...
    if (json['version.CommitmentAdd']) {
      return `Added Commitment: ${Amount.fromPlanck(
        json.amountNQT,
      ).toString()}`;
    }
    ...
    if (
      json['version.MultiOutCreation'] ||
      json['version.MultiSameOutCreation']
    ) {
